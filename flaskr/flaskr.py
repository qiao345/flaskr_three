...
# # csdn 找的解决：sqlite3.OperationalError: no such table: entries
...
def init_db():
    '''此函数用于创建数据库，需要在 flask shell 里引入执行
    '''
    # db_conn 函数的返回值是 sqlite3.connet 方法的返回值
    # sqlite3.connect 方法的返回值是具有 __enter__ 和 __exit__ 两个特殊方法的上下文对象
    # 这个上下文对象也叫连接对象，它的存在就是和 sqlite3 数据库保持连接的标签
    # 其中 as 关键字后面的 conn 就是连接对象，该对象有一个 close 方法用于关闭连接
    # 此处使用 with 关键字处理 sqlite3.connet 方法的返回值
    ...
    with db_conn() as conn:
        ...
        with app.open_resource('schema.sql') as f:
            # 连接对象 conn 的 cursor 方法的返回值是一个光标对象，用于执行sql 语句
            # 该光标对象通常使用 execute 执行 SQL 语句，参数为语句的字符串
            # 光标对象的 executescript 可以一次执行对个 sql 语句
...
def before():
    ...
    '''
    # Flask 应用中有两种上下文对象：用用上下文对象和请求上下文对象
